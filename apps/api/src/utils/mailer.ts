...
// ── Kurs-Enrollment E-Mails ───────────────────────────────────────────────────

export async function sendCourseEnrollmentConfirmEmail(opts: {
  to: string;
  ...
  const { to, participantName, courseTitle, runLabel, confirmLink } = opts;
  const runPart = runLabel ? ` (${runLabel})` : '';
  await getResend().emails.send({
    from: FROM,
    to,
    subject: `Bitte bestätige deine Kursanmeldung: ${courseTitle}`,
    html: `
      <div style="font-family:sans-serif;max-width:540px;margin:0 auto;color:#1a1a1a">
        <h2 style="color:#2563eb">Anmeldung bestätigen</h2>
        <p>Hallo ${participantName},</p>
        <p>
          Danke für deine Anmeldung zum Kurs <strong>${courseTitle}${runPart}</strong>.
        ...
        <p style="color:#6b7280;font-size:13px">
          Dieser Link ist 48 Stunden gültig. Wenn du diese Anmeldung nicht vorgenommen hast, kannst du diese E-Mail ignorieren.
        </p>
        <hr style="border:none;border-top:1px solid #e5e7eb;margin:32px 0"/>
        <p style="color:#9ca3af;font-size:12px">Revio · noreply@my-revio.de</p>
      </div>
...
export async function sendCourseEnrollmentSuccessEmail(opts: {
  to: string;
  participantName: string;
  courseTitle: string;
  runLabel: string | null;
  ...
  const { to, participantName, courseTitle, runLabel, cancelLink, sessions } = opts;
  const runPart = runLabel ? ` (${runLabel})` : '';
  const fmt = (d: Date) =>
    d.toLocaleString('de-DE', { weekday: 'short', day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' });
  const sessionList = sessions
    .slice(0, 10)
    .map(s => `<li>${fmt(s.startsAt)} – ${fmt(s.endsAt)}</li>`)
    .join('');
  const moreNote = sessions.length > 10 ? `<li>… und ${sessions.length - 10} weitere Termine</li>` : '';

  await getResend().emails.send({
    from: FROM,
    to,
    subject: `Anmeldung bestätigt: ${courseTitle}`,
    html: `
      <div style="font-family:sans-serif;max-width:540px;margin:0 auto;color:#1a1a1a">
        <h2 style="color:#16a34a">Anmeldung bestätigt</h2>
        <p>Hallo ${participantName},</p>
        <p>Deine Anmeldung zum Kurs <strong>${courseTitle}${runPart}</strong> ist eingegangen und wird vom Anbieter geprüft.</p>
        ${sessionList ? `<ul style="padding-left:20px">${sessionList}${moreNote}</ul>` : ''}
        <p>
          Möchtest du deine Anmeldung stornieren?
          <a href="${cancelLink}" style="color:#dc2626">Anmeldung stornieren</a>
        </p>
        <hr style="border:none;border-top:1px solid #e5e7eb;margin:32px 0"/>
        <p style="color:#9ca3af;font-size:12px">Revio · noreply@my-revio.de</p>
      </div>
...
export async function sendCourseEnrollmentCancelledEmail(opts: {
  to: string;
  participantName: string;
  courseTitle: string;
  runLabel: string | null;
  cancelledBy: 'PARTICIPANT' | 'PROVIDER';
}) {
  const { to, participantName, courseTitle, runLabel, cancelledBy } = opts;
  const runPart = runLabel ? ` (${runLabel})` : '';
  const reason =
    cancelledBy === 'PROVIDER'
      ? 'Der Anbieter hat deine Anmeldung leider storniert.'
      : 'Deine Anmeldung wurde erfolgreich storniert.';

  await getResend().emails.send({
    from: FROM,
    to,
    subject: `Anmeldung storniert: ${courseTitle}`,
    html: `
      <div style="font-family:sans-serif;max-width:540px;margin:0 auto;color:#1a1a1a">
        <h2 style="color:#dc2626">Anmeldung storniert</h2>
        <p>Hallo ${participantName},</p>
        <p>${reason}</p>
        <p><strong>Kurs:</strong> ${courseTitle}${runPart}</p>
        <p>Bei Fragen erreichst du uns unter <a href="mailto:revioclub.app@gmail.com" style="color:#2563eb">revioclub.app@gmail.com</a>.</p>
        <hr style="border:none;border-top:1px solid #e5e7eb;margin:32px 0"/>
        <p style="color:#9ca3af;font-size:12px">Revio · noreply@my-revio.de</p>
      </div>
    `,
...
export async function sendCourseRunCancelledBulkEmail(opts: {
  to: string;
  participantName: string;
  courseTitle: string;
  runLabel: string | null;
  ...
  const { to, participantName, courseTitle, runLabel, cancelReason } = opts;
  const runPart = runLabel ? ` (${runLabel})` : '';

  await getResend().emails.send({
    from: FROM,
    to,
    subject: `Kurs abgesagt: ${courseTitle}`,
    html: `
      <div style="font-family:sans-serif;max-width:540px;margin:0 auto;color:#1a1a1a">
        <h2 style="color:#dc2626">Kurs wurde abgesagt</h2>
        <p>Hallo ${participantName},</p>
        <p>
          Der Kursdurchlauf <strong>${courseTitle}${runPart}</strong> wurde leider abgesagt.
        </p>
        <p><strong>Grund:</strong> ${cancelReason}</p>
        <p>Bei Fragen erreichst du uns unter <a href="mailto:revioclub.app@gmail.com" style="color:#2563eb">revioclub.app@gmail.com</a>.</p>
        <hr style="border:none;border-top:1px solid #e5e7eb;margin:32px 0"/>
        <p style="color:#9ca3af;font-size:12px">Revio · noreply@my-revio.de</p>
      </div>
