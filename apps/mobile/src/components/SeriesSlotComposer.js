...
export function SeriesSlotComposer({ c, onAddSlots }) {
    ...
    <View style={{ gap: 16 }}>
      ...
      <View style={{ gap: 10 }}>
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
          ...
          <Pressable
            ...
            style={{ flexDirection: 'row', alignItems: 'center', gap: 6, paddingVertical: 8, paddingHorizontal: 12, borderRadius: RADIUS.sm, borderWidth: 1.5, borderColor: c.border, backgroundColor: c.mutedBg }}
          >
      ...
      <View style={{ backgroundColor: c.primaryBg, borderRadius: RADIUS.sm, padding: 10, alignItems: 'center' }}>
        <Text style={{ fontSize: 13, fontWeight: '600', color: c.primary }}>
          {count === 0 ? 'Keine Termine in diesem Zeitraum' : count === 1 ? '1 Termin wird angelegt' : `${count} Termine werden angelegt`}
        </Text>
      </View>
      ...
      {overLimit ? (
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: c.errorBg ?? '#FEF2F2', borderRadius: RADIUS.sm, padding: 10 }}>
          ...
          <Text style={{ fontSize: 12, color: c.error, flex: 1 }}>Maximal {MAX_SLOTS} Termine pro Serie — bitte Zeitraum eingrenzen.</Text>
        </View>
      ) : null}
      ...
      <Pressable
        ...
        disabled={!canSubmit}
        style={{ backgroundColor: canSubmit ? c.primary : c.border, borderRadius: RADIUS.sm, paddingVertical: 12, alignItems: 'center' }}
      >
        <Text style={{ color: '#fff', fontSize: 14, fontWeight: '600' }}>
          {canSubmit ? `Serie anlegen · ${count} Termine` : 'Serie anlegen'}
        </Text>
      </Pressable>
