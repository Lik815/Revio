...
export function TherapistAppointmentDetail({
  ...
  const cardContent = (
    <View style={{ backgroundColor: c.card, borderRadius: 24, padding: 18, borderWidth: 1, borderColor: c.border, gap: 14, ...SHADOW.card }}>
      ...
      <View style={{ gap: 8 }}>
        ...
        {patient?.phone ? (
          <Pressable onPress={() => Linking.openURL(`tel:${patient.phone}`)} style={{ flexDirection: 'row', alignItems: 'center', gap: 5 }}>
            <Ionicons name="call" size={13} color={c.primary} />
            <Text style={{ fontSize: 13, color: c.primary, fontWeight: '600' }}>{patient.phone}</Text>
          </Pressable>
        ) : null}
        {patient?.email ? (
          <Pressable onPress={() => Linking.openURL(`mailto:${patient.email}`)} style={{ flexDirection: 'row', alignItems: 'center', gap: 5 }}>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'flex-start', gap: 5 }}>
          ...
          <Text style={{ fontSize: 13, color: c.muted }}>{patient?.addressLine ?? 'Adresse noch nicht verfügbar'}</Text>
        </View>
      </View>
      ...
      {heilmittelLabel || kassenartLabel ? (
        <>
          ...
          <View style={{ gap: 6 }}>
            ...
            <Text style={{ fontSize: 14, lineHeight: 21, color: c.text }}>{[heilmittelLabel, kassenartLabel].filter(Boolean).join(' · ')}</Text>
          </View>
        </>
      ) : null}
  ...
  if (isModal) {
    return (
      <View style={{ flex: 1, backgroundColor: c.card, borderTopLeftRadius: 20, borderTopRightRadius: 20 }}>
